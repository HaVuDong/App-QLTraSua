...
import {
  getPayrollBackendStatusLabel,
  getPayrollSourceLabel,
  getStaffRoleLabel,
...
const PAYROLL_STATUS_FILTER_OPTIONS: PayrollFilterStatus[] = [
  "ALL",
  "ACTIVE",
  "LOCKED",
  "DELETED",
...
function getPayrollRecordUserId(record: PayrollRecord) {
  ...
  if (record.userId && typeof record.userId._id === "string")
...
function getAttendanceRecordHours(
  record: NonNullable<MonthlyAttendanceData["records"]>[number],
) {
  if (
...
export function PayrollManagementScreen() {
  ...
  const canCalculatePayroll = canViewPayroll;
  const canManagePayroll = hasPermission(user, "PAYROLL_CONFIRM");
  ...
  const fetchPayrollData = useCallback(
    async (isRefresh = false) => {
      try {
        ...
        const staffUsers = staffRows.filter(
          (member) => member.role !== "SYSTEM_OWNER" && member.role !== "ADMIN",
        );
        const staffMap = new Map(
          staffUsers.map((member) => [member._id, member] as const),
        );
        ...
        try {
          ...
          if (statusCode === 404) {
            setPayrollApiNotice(
              "Chua co ban ghi bang luong cho thang nay. Co the bam Tinh luong de tao du lieu.",
            );
          } else {
            setPayrollApiNotice(
              "Khong the tai bang luong tu may chu. Dang hien thi luong tam tinh tu cham cong.",
            );
          }
        }
        ...
        const userIds = Array.from(
          new Set([
            ...
            ...payrollRows
              .map((record) => getPayrollRecordUserId(record))
              .filter((recordUserId) => !!recordUserId),
          ]),
        );
        ...
        const monthlyEntries = await Promise.all(
          userIds.map(async (targetUserId) => {
            try {
              const monthly = await getMonthlyAttendance(
                targetUserId,
                appliedMonth,
              );
        ...
        const mappedRows: PayrollViewRow[] = userIds.map((targetUserId) => {
          ...
          const payrollUserRef =
            payrollRecord && typeof payrollRecord.userId !== "string"
              ? payrollRecord.userId
              : null;

          const role = normalizeToStaffRole(
            staffMember?.role || payrollUserRef?.role,
          );
          const status: PayrollViewRow["status"] =
            staffMember?.status || "UNKNOWN";
          const account =
            staffMember?.email ||
            staffMember?.phone ||
            payrollUserRef?.email ||
            "";
          const name = staffMember?.name || payrollUserRef?.name || "Nhân viên";
          const hourlyWage =
            typeof staffMember?.salaryConfig?.baseHourly === "number"
              ? staffMember.salaryConfig.baseHourly
              : null;

          const workedHoursFromRecords =
            monthlyAttendance?.records?.reduce(
              (sum, record) => sum + getAttendanceRecordHours(record),
              0,
            ) || 0;
          const workedHours =
            payrollRecord && Number.isFinite(payrollRecord.workedHours)
              ...
              : monthlyAttendance &&
                  Number.isFinite(monthlyAttendance.totalWorkedHours)
          ...
          const attendancePenaltyRecords = (monthlyAttendance?.records || [])
            .filter(
              (record) =>
                (record.lateMinutes || 0) >= PAYROLL_LATE_MINUTES_THRESHOLD,
            )
            .map((record) => ({
              date: record.date,
          ...
          const backendPenaltyRecords = extractBackendLatePenaltyRecords(
            payrollRecord?.deductions,
          );
          const hasBackendPenaltyData =
            !!payrollRecord && backendPenaltyRecords.length > 0;
          ...
          const penaltySource: PayrollViewRow["penaltySource"] =
            hasBackendPenaltyData ? "BACKEND" : "ATTENDANCE";
          const latePenaltyTotal = penaltyRecords.reduce(
            (sum, record) => sum + record.penaltyAmount,
            0,
          );
          const grossSalary =
            hourlyWage === null ? null : Math.round(workedHours * hourlyWage);
  ...
  const applyMonth = useCallback(() => {
    ...
    if (!/^\d{4}-\d{2}$/.test(normalizedMonth)) {
      Alert.alert(
        "Tháng không hợp lệ",
        "Vui lòng nhập tháng theo định dạng YYYY-MM",
      );
      return;
    }
    ...
    runConfirmedAction({
      ...
      onConfirm: async () => {
        try {
          ...
          Alert.alert(
            "Thành công",
            result?.message || `Đã gửi tính lương tháng ${appliedMonth}`,
          );
          void fetchPayrollData(true);
        } catch (err: any) {
          Alert.alert(
            "Lỗi",
            err.response?.data?.message || "Không thể tính lương",
          );
        } finally {
    ...
    runConfirmedAction({
      ...
      onConfirm: async () => {
        try {
          ...
          Alert.alert(
            "Thanh cong",
            result?.message || `Da xac nhan bang luong thang ${appliedMonth}`,
          );
          void fetchPayrollData(true);
        } catch (err: any) {
          Alert.alert(
            "Loi",
            err.response?.data?.message || "Khong the xac nhan bang luong",
          );
        } finally {
          setConfirming(false);
        }
  ...
  const handleExportPayroll = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        "Loi",
        err.response?.data?.message || "Khong the xuat bang luong",
      );
    } finally {
      setExporting(false);
    }
  ...
  const handleAdjustPayroll = useCallback(async () => {
    ...
    if (
      (allowanceAmount &&
        (!Number.isFinite(allowanceValue) || allowanceValue < 0)) ||
      (deductionAmount &&
        (!Number.isFinite(deductionValue) || deductionValue < 0))
    ) {
    ...
    try {
      ...
      const nextPayroll = await adjustPayroll(detailPayroll._id, {
        allowances:
          allowanceValue > 0
            ? [
                {
                  name: allowanceName.trim() || "Dieu chinh phu cap",
        ...
        deductions:
          deductionValue > 0
            ? [
                {
                  name: deductionName.trim() || "Dieu chinh khau tru",
      ...
      Alert.alert(
        "Loi",
        err.response?.data?.message || "Khong the dieu chinh bang luong",
      );
    } finally {
      setAdjusting(false);
    }
  ...
  const openPayrollDetail = useCallback(
    async (row: PayrollViewRow) => {
      ...
      try {
        ...
        if (statusCode === 404) {
          setDetailError(
            "Chua co ban ghi bang luong cho nhan vien nay trong thang da chon.",
          );
        } else {
          setDetailError(
            err.response?.data?.message || "Không thể tải chi tiết lương",
          );
        }
      } finally {
  ...
  const totalGrossSalary = filteredRows.reduce(
    (sum, row) => sum + (row.grossSalary || 0),
    0,
  );
  const totalPenalty = filteredRows.reduce(
    (sum, row) => sum + row.latePenaltyTotal,
    0,
  );
  const totalFinalPreview = filteredRows.reduce(
    (sum, row) => sum + (row.finalSalaryPreview || 0),
    0,
  );

  const selectedRow = detailUserId
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl
            ...
            onRefresh={() => void fetchPayrollData(true)}
          />
        }
      >
              ...
              {canCalculatePayroll ? (
                ...
                >
                  {calculating ? (
                    ...
                    <Text style={styles.buttonText}>Tính lương nhân viên</Text>
                  )}
                </TouchableOpacity>
              ) : (
                <TouchableOpacity
            ...
            <View style={styles.rowSplit}>
              {canManagePayroll ? (
                ...
                >
                  {confirming ? (
                    ...
                    <Text style={styles.buttonText}>Xac nhan thang</Text>
                  )}
                </TouchableOpacity>
              ) : null}
              ...
              <TouchableOpacity
                ...
                style={[
                  ...
                  styles.buttonSecondary,
                  styles.flex1,
                  !canExportPayroll ? styles.moduleCardDisabled : null,
                ]}
              ...
              >
                {exporting ? (
                  ...
                  <Text style={styles.buttonText}>
                    {canExportPayroll ? "Xuat Excel" : "Xuat tren web"}
                  </Text>
                )}
              </TouchableOpacity>
            </View>
              ...
              {PAYROLL_STATUS_FILTER_OPTIONS.map((status) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {getPayrollFilterStatusLabel(status)}
                    </Text>
                  </TouchableOpacity>
                );
              })}
          ...
          {payrollApiNotice ? (
            ...
            <View style={[styles.glassCard, styles.disabledBlock]}>
              <Text style={styles.helperText}>
                Ưu tiên dữ liệu bảng lương từ máy chủ khi có. Nếu chưa có, hệ
                thống tạm tính theo quy tắc đi trễ từ 5 phút trở lên trừ 20,000
                VND/lần.
              </Text>
            </View>
          )}
              ...
              return (
                ...
                >
                  <View style={styles.staffHeader}>
                    <View style={styles.staffInfo}>
                      ...
                      <Text style={styles.staffMeta}>
                        Vai trò: {getStaffRoleLabel(row.role)}
                      </Text>
                      <Text style={styles.staffMeta}>
                      ...
                      <Text style={styles.staffMeta}>
                        ...
                        {row.hourlyWage === null
                          ? "Chưa cấu hình lương"
                          : formatCurrencyVnd(row.hourlyWage)}
                      </Text>
                      <Text style={styles.staffMeta}>
                        Tổng giờ làm: {Number(row.workedHours.toFixed(2))}
                      </Text>
                      <Text style={styles.staffMeta}>
                        Lương tạm tính:{" "}
                      ...
                      <Text style={styles.staffMeta}>
                        Tổng phạt đi trễ:{" "}
                        {formatCurrencyVnd(row.latePenaltyTotal)}
                      </Text>
                      <Text style={styles.staffMeta}>
                        Nguồn dữ liệu phạt đi trễ:{" "}
                        {getPayrollSourceLabel(row.penaltySource)}
                      </Text>
                      <Text style={styles.staffMeta}>
                        Lương thực nhận:{" "}
                        {row.finalSalaryPreview === null
                          ? "Chưa cấu hình lương"
                          : formatCurrencyVnd(row.finalSalaryPreview)}
                      </Text>
                      {row.payrollRecord ? (
                        <Text style={styles.staffMeta}>
                          Lương từ máy chủ:{" "}
                          {formatCurrencyVnd(
                            row.payrollRecord.finalSalary || 0,
                          )}{" "}
                          (
                          {getPayrollBackendStatusLabel(
                            row.payrollRecord.status,
                          )}
                          )
                        </Text>
                      ) : (
                        <Text style={styles.staffMeta}>
                          Chua co ban ghi luong tu may chu trong thang nay
                        </Text>
                      )}
                    </View>
                    <View style={[styles.statusBadge, statusStyle]}>
                      <Text style={styles.statusText}>
                        {getPayrollRowStatusLabel(row.status)}
                      </Text>
                    </View>
                  </View>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => void openPayrollDetail(row)}
                  >
          ...
          {selectedRow ? (
            <View style={[styles.glassCard, styles.formCard]}>
              <Text style={styles.sectionTitle}>
                Chi tiết lương - {selectedRow.name}
              </Text>
              <Text style={styles.staffMeta}>Lương tháng: {appliedMonth}</Text>
              <Text style={styles.staffMeta}>
                Vai trò: {getStaffRoleLabel(selectedRow.role)}
              </Text>
              <Text style={styles.staffMeta}>
                Lương theo giờ:{" "}
                {selectedRow.hourlyWage === null
                  ? "Chưa cấu hình lương"
                  : formatCurrencyVnd(selectedRow.hourlyWage)}
              </Text>
              <Text style={styles.staffMeta}>
                Tổng giờ làm: {Number(selectedRow.workedHours.toFixed(2))}
              </Text>
              <Text style={styles.staffMeta}>
                Lương tạm tính:{" "}
                {selectedRow.grossSalary === null
                  ? "Chưa cấu hình lương"
                  : formatCurrencyVnd(selectedRow.grossSalary)}
              </Text>
              <Text style={styles.staffMeta}>
                Tổng phạt đi trễ:{" "}
                {formatCurrencyVnd(selectedRow.latePenaltyTotal)}
              </Text>
              <Text style={styles.staffMeta}>
                Lương thực nhận:{" "}
                {selectedRow.finalSalaryPreview === null
                  ? "Chưa cấu hình lương"
                  : formatCurrencyVnd(selectedRow.finalSalaryPreview)}
              </Text>

              {detailLoading ? <LoadingView /> : null}
              {detailError ? (
                <Text style={styles.otpNotice}>{detailError}</Text>
              ) : null}

              {detailPayroll ? (
                <View style={[styles.glassCard, styles.disabledBlock]}>
                  <Text style={styles.sectionTitle}>
                    Dữ liệu bảng lương từ máy chủ
                  </Text>
                  <Text style={styles.staffMeta}>
                    Trạng thái:{" "}
                    {getPayrollBackendStatusLabel(detailPayroll.status)}
                  </Text>
                  <Text style={styles.staffMeta}>
                    Giờ làm việc: {detailPayroll.workedHours || 0}
                  </Text>
                  <Text style={styles.staffMeta}>
                    Tổng khấu trừ từ máy chủ:{" "}
                    {formatCurrencyVnd(detailPayroll.totalDeductions || 0)}
                  </Text>
                  <Text style={styles.staffMeta}>
                    Lương từ máy chủ:{" "}
                    {formatCurrencyVnd(detailPayroll.finalSalary || 0)}
                  </Text>
                </View>
              ) : null}
              ...
              {selectedRow.penaltyRecords.length === 0 ? (
                ...
                selectedRow.penaltyRecords.map((penalty, idx) => (
                  <View
                    key={`${selectedRow.userId}-penalty-${idx}`}
                    style={[styles.glassCard, styles.staffCard]}
                  >
                    <Text style={styles.staffMeta}>
                      Ngày: {formatPenaltyDateLabel(penalty.date)}
                    </Text>
                    <Text style={styles.staffMeta}>
                      Gio vao ca: Chua co du lieu ca lam
                    </Text>
                    <Text style={styles.staffMeta}>
                      Giờ check-in: {formatDateTime(penalty.checkInTime)}
                    </Text>
                    <Text style={styles.staffMeta}>
                      Đi trễ: {penalty.lateMinutes} phút
                    </Text>
                    <Text style={styles.staffMeta}>
                      Khoản trừ đi trễ:{" "}
                      {formatCurrencyVnd(penalty.penaltyAmount)}
                    </Text>
                    <Text style={styles.staffMeta}>
                      Trạng thái:{" "}
                      {selectedRow.penaltySource === "BACKEND"
                        ? "Tính từ API bảng lương"
                        : "Tính từ dữ liệu chấm công"}
                    </Text>
                  </View>
                ))
              )}
              ...
              {canManagePayroll && detailPayroll ? (
                <View style={[styles.glassCard, styles.disabledBlock]}>
                  ...
                  <TouchableOpacity
                    activeOpacity={0.8}
                    style={[styles.buttonBase, styles.buttonPrimary]}
                    onPress={() => void handleAdjustPayroll()}
                  >
                    {adjusting ? (
                      <ActivityIndicator color={COLORS.text} />
                    ) : (
                      <Text style={styles.buttonText}>Luu dieu chinh</Text>
                    )}
                  </TouchableOpacity>
                </View>
              ) : null}
              ...
              <Text style={styles.sectionTitle}>
                Bản ghi chấm công dùng để tính lương
              </Text>
              {detailAttendanceRecords.length === 0 ? (
                ...
                detailAttendanceRecords.map((record) => {
                  ...
                  const penaltyApplied =
                    lateMinutes >= PAYROLL_LATE_MINUTES_THRESHOLD;
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text style={styles.staffMeta}>
                        Giờ check-in: {formatDateTime(record.checkInTime)}
                      </Text>
                      <Text style={styles.staffMeta}>
                        Giờ check-out: {formatDateTime(record.checkOutTime)}
                      </Text>
                      <Text style={styles.staffMeta}>
                        Tổng giờ làm: {recordHours}
                      </Text>
                      <Text style={styles.staffMeta}>
                        Đi trễ: {lateMinutes} phút
                      </Text>
                      <Text style={styles.staffMeta}>
                        Khoản trừ áp dụng:{" "}
                        {penaltyApplied
                          ? formatCurrencyVnd(PAYROLL_LATE_PENALTY_AMOUNT)
                          : "Không"}
                      </Text>
                    </View>
                  );
