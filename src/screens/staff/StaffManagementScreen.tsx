...
import { changeStaffRole, createStaffUser, deactivateStaffUser, getStaffUsers, lockStaffUser, resetStaffPassword, unlockStaffUser, updateStaffUser, type StaffRole, type StaffStatus, type StaffUser } from '../../services/staff';
import { styles } from '../../styles/appStyles';
import { COLORS } from '../../theme';
import { ADMIN_STAFF_CREATE_ROLE_OPTIONS, MANAGER_STAFF_CREATE_ROLE_OPTIONS, STAFF_EDIT_ROLE_OPTIONS, STAFF_STATUS_FILTER_OPTIONS, type StaffEditableRole, type StaffFilterRole, type StaffFilterStatus } from '../../utils/staffForms';
...
function getStaffCreateRoleOptions(currentRole?: StaffRole | null): StaffEditableRole[] {
...
export function StaffManagementScreen() {
  ...
  const staffCreateRoleOptions = getStaffCreateRoleOptions(user?.role as StaffRole | undefined);
  ...
  const fetchStaff = useCallback(async (isRefresh = false) => {
    try {
      ...
      setLoadError(err.response?.data?.message || 'Không thể tải dữ liệu nhân sự');
    } finally {
  ...
  const handleToggleLock = useCallback((member: StaffUser) => {
    if (!canAdminManageStaff) {
      Alert.alert('Không đủ quyền', 'Cần quyền ADMIN để khóa/mở khóa tài khoản.');
      return;
    }
    if (!user) return;
    ...
    if (member.status !== 'LOCKED' && member.role === 'ADMIN') {
      Alert.alert('Không hợp lệ', 'Không khóa tài khoản Admin để tránh mất quyền quản trị.');
      return;
    }
    ...
    runConfirmedAction({
      ...
      message: isLocked ? `Bạn muốn mở khóa tài khoản ${member.name}?` : `Bạn muốn khóa tài khoản ${member.name}?`,
      ...
      onConfirm: async () => {
        try {
          ...
          Alert.alert('Lỗi', err.response?.data?.message || 'Không thể cập nhật trạng thái tài khoản');
        }
      },
    });
  ...
  const handleDeactivate = useCallback((member: StaffUser) => {
    if (!canAdminManageStaff) {
      Alert.alert('Không đủ quyền', 'Cần quyền ADMIN để vô hiệu hóa nhân viên.');
      return;
    }
    if (!user) return;
    if (member._id === user.userId) {
      Alert.alert('Không hợp lệ', 'Không thể vô hiệu hóa tài khoản đang đăng nhập.');
      return;
    }
    ...
    runConfirmedAction({
      ...
      onConfirm: async () => {
        try {
          ...
          Alert.alert('Lỗi', err.response?.data?.message || 'Không thể vô hiệu hóa nhân viên');
        }
      },
    });
  }, [canAdminManageStaff, fetchStaff, user]);
  ...
  const handleResetPassword = useCallback((member: StaffUser) => {
    ...
    if (member.role === 'SYSTEM_OWNER') {
      Alert.alert('Không hợp lệ', 'Không reset mật khẩu SYSTEM_OWNER tại đây.');
      return;
    }
    ...
    runConfirmedAction({
      ...
      onConfirm: async () => {
        try {
          ...
          Alert.alert('Lỗi', err.response?.data?.message || 'Không thể reset mật khẩu');
        }
      },
    });
  }, [canAdminManageStaff]);
  ...
  const filteredStaff = staff.filter((member) => {
    ...
    const searchable = `${member.name || ''} ${username} ${member.email || ''} ${member.phone || ''} ${member.role}`.toLowerCase();
      ...
      >
        <View style={styles.screenStack}>
          ...
          <View style={[styles.glassCard, styles.inventoryToolbar]}>
            ...
            <View style={styles.filterRow}>
              {STAFF_STATUS_FILTER_OPTIONS.map((status) => {
                ...
                return (
                  ...
                  >
                    <Text style={[styles.filterChipText, selected ? styles.filterChipTextActive : null]}>
                      {status === 'ALL' ? 'Tất cả' : getStaffStatusLabel(status)}
                    </Text>
                  </TouchableOpacity>
                );
              })}
            ...
            {!canCreateStaff ? <Text style={styles.helperText}>Bạn không có quyền tạo nhân viên.</Text> : null}
            {!canAdminManageStaff && canCreateStaff ? (
              <Text style={styles.helperText}>MANAGER chỉ được tạo nhân viên và bếp; các thao tác sửa/khóa/reset cần ADMIN.</Text>
            ) : null}
          </View>

          {formMode ? (
            <View style={[styles.glassCard, styles.formCard]}>
              <Text style={styles.sectionTitle}>{formMode === 'create' ? 'Thêm nhân viên' : 'Sửa nhân viên'}</Text>
              ...
              <View style={styles.filterRow}>
                {(formMode === 'create' ? staffCreateRoleOptions : STAFF_EDIT_ROLE_OPTIONS).map((role) => {
              ...
              {formMode === 'create' ? (
                <Text style={styles.helperText}>Máy chủ sẽ tạo mật khẩu tạm thời nếu không nhập mật khẩu riêng.</Text>
              ) : null}
              ...
              <View style={styles.rowSplit}>
                ...
                <TouchableOpacity activeOpacity={0.8} style={[styles.buttonBase, styles.buttonPrimary, styles.flex1]} onPress={() => void saveStaffForm()}>
                  {formSubmitting ? <ActivityIndicator color={COLORS.text} /> : <Text style={styles.buttonText}>Lưu nhân viên</Text>}
                </TouchableOpacity>
              </View>
            </View>
          ) : null}
          ...
          {!loadError && filteredStaff.length === 0 ? (
            ...
            filteredStaff.map((member) => {
              ...
              const username = getStaffLoginIdentifier(member) || 'Chưa có tên tài khoản';
              const canEdit = canAdminManageStaff && member.role !== 'SYSTEM_OWNER' && member.role !== 'ADMIN';
              const canDeactivate =
                canAdminManageStaff && member._id !== user.userId && member.role !== 'SYSTEM_OWNER' && member.role !== 'ADMIN' && member.status !== 'DELETED';
              const canResetPassword = canAdminManageStaff && member.role !== 'SYSTEM_OWNER';
              const canToggleLock =
                canAdminManageStaff &&
                ...
                <View key={member._id} style={[styles.glassCard, styles.staffCard]}>
                  <View style={styles.staffHeader}>
                    <View style={styles.staffInfo}>
                      <Text style={styles.staffName}>{member.name || 'Nhân viên'}</Text>
                      <Text style={styles.staffMeta}>Email tài khoản: {username}</Text>
                      <Text style={styles.staffMeta}>Số điện thoại: {member.phone || '-'}</Text>
                      <Text style={styles.staffMeta}>Vai trò: {getStaffRoleLabel(member.role)}</Text>
                      <Text style={styles.staffMeta}>Vị trí làm việc: {getStaffRoleLabel(member.role)}</Text>
                      ...
                      <Text style={styles.staffMeta}>Tạo lúc: {member.createdAt ? new Date(member.createdAt).toLocaleString() : '-'}</Text>
                    </View>
                  ...
                  <View style={styles.inventoryActionRow}>
                    ...
                    {canToggleLock ? (
                      ...
                      >
                        <Text style={styles.buttonText}>{member.status === 'LOCKED' ? 'Mở khóa tài khoản' : 'Khóa tài khoản'}</Text>
                      </TouchableOpacity>
                    ) : (
                      <TouchableOpacity activeOpacity={1} disabled style={[styles.buttonBase, styles.buttonSecondary, styles.flex1, styles.moduleCardDisabled]}>
                        <Text style={styles.buttonText}>{member.status === 'LOCKED' ? 'Mở khóa tài khoản' : 'Khóa tài khoản'}</Text>
